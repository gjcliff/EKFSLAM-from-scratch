...
TEST_CASE("Test * operator with a Vector2D object and a double", "[operator*_vector]")
{
  Vector2D v = {1.0, 1.0};
  Vector2D g = v * 2.0;
  REQUIRE_THAT(g.x, Catch::Matchers::WithinAbs(2.0, 0.0001));
  REQUIRE_THAT(g.y, Catch::Matchers::WithinAbs(2.0, 0.0001));
}
...
TEST_CASE("Test + operator with a Vector2D object and a double", "[operator+_vector]")
{
  ...
  Vector2D x = {1.0, 1.0};
  Vector2D y = v + x;
  REQUIRE_THAT(y.x, Catch::Matchers::WithinAbs(2.0, 0.0001));
  REQUIRE_THAT(y.y, Catch::Matchers::WithinAbs(2.0, 0.0001));
}
...
TEST_CASE("Test + operator with a Vector2D object and a double", "[operator-_vector]")
{
  ...
  Vector2D x = {1.0, 1.0};
  Vector2D y = v - x;
  REQUIRE_THAT(y.x, Catch::Matchers::WithinAbs(0.0, 0.0001));
  REQUIRE_THAT(y.y, Catch::Matchers::WithinAbs(0.0, 0.0001));
}
...
TEST_CASE("Test + operator with a Vector2D object and a double", "[operator-=_vector]")
{
  ...
  Vector2D x = {1.0, 1.0};
  v -= x;
  REQUIRE_THAT(v.x, Catch::Matchers::WithinAbs(0.0, 0.0001));
  REQUIRE_THAT(v.y, Catch::Matchers::WithinAbs(0.0, 0.0001));
}
...
TEST_CASE("Test the dot() function", "[dot]")
{
  ...
  Vector2D x = {1.0, 1.0};
  double y = dot(v, x);
  REQUIRE_THAT(y, Catch::Matchers::WithinAbs(2.0, 0.0001));
}

TEST_CASE("Test the magnitude() function", "[magnitude]")
{
  Vector2D v = {3.0, 4.0};
  double y = magnitude(v);
  REQUIRE_THAT(y, Catch::Matchers::WithinAbs(5.0, 0.0001));
}

TEST_CASE("Test the angle() function", "[angle]")
{
  Vector2D v = {1.0, 0.0};
  Vector2D x = {0.0, 1.0};
  double y = angle(v, x);
  REQUIRE_THAT(y, Catch::Matchers::WithinAbs(1.5708, 0.0001));
}
}
